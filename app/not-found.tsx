...
export default function NotFound() {
  return (
    <div className="mx-auto max-w-xl space-y-4 py-12 text-center">
      <p className="text-5xl font-semibold tabular-nums text-zinc-500 dark:text-zinc-400">
        404
      </p>
      ...
      <div className="flex flex-wrap justify-center gap-2 pt-2">
        {LINKS.map((l) => (
          <Link
            key={l.href}
            href={l.href}
            className="rounded-md border border-black/10 bg-white px-3 py-1.5 text-sm font-medium transition-colors hover:border-brand-300 hover:text-brand-600 dark:hover:text-brand-400 dark:border-white/10 dark:bg-zinc-900 dark:hover:border-brand-800 dark:hover:text-brand-400"
          >
            {l.label}
          </Link>
        ))}
